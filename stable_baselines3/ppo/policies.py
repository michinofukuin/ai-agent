...
class ICM(object):
    def __init__(self,
        ...
        if envs.action_space.__class__.__name__ == "Discrete":
            ...
            self.inverse_forward_model = InverseForwardDynamicsModel(
                kwargs={'latent_dim': 64, 'action_dim': self.action_shape}
            ).to(device)
            ...
            self.inverse_forward_model = InverseForwardDynamicsModel(
                kwargs={'latent_dim': 64, 'action_dim': 7}
            ).to(device)
        ...
        if len(self.ob_shape) == 3:
            self.cnn_encoder = CnnEncoder(kwargs={'in_channels': 3}).to(device)
    ...
    def update(self, rollouts):
        ...
        obs = torch.Tensor(rollouts.observations).reshape(n_steps, *self.ob_shape)
        if self.action_type == 'dis':
            actions = torch.Tensor(rollouts.actions).reshape(n_steps, )
            actions = F.one_hot(actions.to(torch.int64), self.action_shape).float()
        else:
            actions = torch.Tensor(rollouts.actions).reshape(n_steps, *self.action_shape)
    ...
    def compute_irs(self, rollouts, time_steps):
        ...
        n_steps = rollouts.observations.shape[0]
        intrinsic_rewards = np.zeros(shape=(n_steps, 1))

        obs = torch.Tensor(rollouts.observations)
        actions = torch.Tensor(rollouts.actions)
        if self.action_type == 'dis':
        ...
        with torch.no_grad():
            for idx in range(1):
                ...
                intrinsic_rewards = F.mse_loss(processed_pred_next_obs, processed_next_obs, reduction='mean').cpu().numpy()
