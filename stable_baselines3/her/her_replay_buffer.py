...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def _get_virtual_samples(
        ...
        if isr==1:
            ...
            g=th.Tensor(next_obs['achieved_goal'])
            gg=th.Tensor(obs['desired_goal'])
            combined_dim = g.size(1) * g.size(2) * g.size(3)
            g_flattened = g.view(g.size(0), combined_dim)
            gg_flattened = gg.view(gg.size(0), combined_dim)
            equal = th.all(g_flattened == gg_flattened, dim=1)
        # 将布尔张量转换为浮点张量，并调整形状为 (9, 1)
            rewards = equal.to(th.float32).unsqueeze(1)
        #rewards = rewards.astype(np.float32)  # env_method returns a list containing one element
